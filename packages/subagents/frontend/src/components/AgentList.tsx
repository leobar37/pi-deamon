...
export function AgentList() {
	...
	// Sync TanStack Query initial load into zustand store
	useEffect(() => {
		if (fetchedAgents) {
	...
	return (
		<div className="flex flex-col h-full">
			<div className="px-4 py-3 border-b border-border-subtle bg-bg-elevated">
				<h1 className="text-base font-semibold text-text-primary">SubAgents</h1>
				<p className="text-xs text-text-muted mt-0.5">
					{isLoading ? "Loading..." : error ? "Error loading agents" : `${agents.length} instance${agents.length === 1 ? "" : "s"}`}
				</p>
			</div>
			<div className="flex-1 overflow-y-auto p-3 space-y-2">
				{sorted.map((agent) => (
					<AgentCard
						...
						onClick={() =>
							selectAgent(agent.instanceId === selectedAgentId ? null : agent.instanceId)
						}
					/>
				))}
