import type { SubAgentState } from "../../store/message-blocks.js";
...
export function StatusBadge({ state, pulse }: StatusBadgeProps) {
	...
	return (
		...
		>
			<span
				className={`w-1.5 h-1.5 rounded-full ${state === "running" ? "bg-accent" : state === "completed" ? "bg-success" : state === "failed" || state === "timed_out" ? "bg-error" : "bg-text-muted"}`}
			/>
			{cfg.label}
		</span>
	);
