import type { SubAgentEvent } from "../../store/message-blocks.js";
...
function getEventContent(event: SubAgentEvent): {
	icon: string;
	title: string;
	body?: string;
	variant: "system" | "agent" | "tool" | "error" | "progress";
} {
	switch (event.type) {
		case "task.start":
			return {
				icon: ">",
				title: "Agent started",
				body: (event as unknown as { description?: string }).description,
				variant: "system",
			};
		case "task.end": {
			const result = (event as unknown as { result?: { status: string; summary: string } }).result;
			return {
				icon: result?.status === "completed" ? "ok" : "x",
				title: `Task ${result?.status ?? "ended"}`,
				body: result?.summary,
				variant: result?.status === "completed" ? "agent" : "error",
			};
		}
		case "turn.complete": {
			...
			return {
				icon: "r",
				title: `Turn ${turn.turnIndex + 1} complete`,
				body: `${turn.toolCount} tool${turn.toolCount === 1 ? "" : "s"}${turn.hadError ? " (had error)" : ""}`,
				variant: "system",
			};
		}
		case "tool.start": {
		...
		case "tool.end": {
			...
			return {
				icon: tool.isError ? "x" : "ok",
				title: `Tool ${tool.isError ? "failed" : "done"}`,
				body: tool.toolName,
				variant: tool.isError ? "error" : "tool",
			};
		}
		case "tool.execute": {
			const tool = event as unknown as { toolName: string; isError: boolean };
		...
		case "error": {
			...
			return {
				icon: "!",
				title: (event as unknown as { fatal?: boolean }).fatal ? "Fatal error" : "Error",
				body: err,
				variant: "error",
			};
		}
		...
		case "summary.available": {
			...
			return {
				icon: "n",
				title: `Summary (${sa.messageCount} messages)`,
				body: sa.summary,
				variant: "agent",
			};
		}
