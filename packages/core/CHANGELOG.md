...
- Package renamed from `@local/pi-subagents` to `@local/pi-core`. Session backend, Lion orchestration, and bundled extensions now ship from a single workspace package.
- Vendored extensions from `packages/extensions/` merged into `packages/core/src/extensions/`.
- Build script unified: `packages/core/build.ts` now produces both library bundles (`dist/index.js`, `dist/sdk/*`, `dist/api/contract.js`) and per-extension bundles (`dist/<name>.js`).
- Frontend package renamed from `@local/pi-subagents-frontend` to `@local/pi-core-frontend`.
- All external consumers (dashboard, dev scripts, tsconfig) updated to import from `@local/pi-core`.

### Added

- Subpath exports for bundled extensions: `@local/pi-core/extensions/<name>`.
- `NOTICE.md` documenting upstream provenance of all 18 vendored extensions.
