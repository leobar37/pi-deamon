...
Session core and web dashboard backend for the [Pi](https://github.com/earendil-works/pi-mono) coding agent. This package hosts the HTTP server, real-time event streaming, standalone agent sessions, Lion orchestration, and bundled Pi extensions. It is consumed by the Pi dashboard as the agent backend.
...
├── / — Static SPA files (core frontend)
│
...
- `/` — Static SPA files (the core frontend)

Each session is accessible at `/thread/<threadId>` where the core frontend renders the session UI.
...
```typescript
import { HttpServerTransport, SubAgentController } from "@local/pi-core";
