	...
	try {
		...
		await tracker.recordProgress(
			...
			},
			{
				...
				relevantFiles: ["packages/core/src/extensions/goal-v2/context-store.ts"],
		...
		assert.deepEqual(doc.relevantFiles, ["packages/core/src/extensions/goal-v2/context-store.ts"]);
...
describe("Goal v2 workflow", () => {
	it("tracks phase and blocked status", testCoreTracksPhaseAndBlockedStatus);
	it("persists structured progress in markdown context", testMarkdownContextPersistsStructuredProgress);
	it("drafting flow", testDraftingFlow);
	it("activates goal tools only after goal command", testGoalToolsActivateOnlyAfterGoalCommand);
	it("stays inactive during Lion build", testGoalToolsStayInactiveDuringLionBuild);
	it("stays inactive during file-backed Lion build", testGoalToolsStayInactiveDuringFileBackedLionBuild);
	it("activates during file-backed Lion planning", testGoalToolsActivateDuringFileBackedLionPlanning);
	it("pauses goal without continuation on aborted agent end", testAbortedAgentEndPausesGoalWithoutContinuation);
	it("blocks goal tool call when inactive", testGoalToolCallBlockedWhenInactive);
	it("records tool errors as progress", testToolErrorsRecordedAsProgress);
	it("ignores non-error tool results", testNonErrorToolResultsIgnored);
	it("migrates legacy JSON", testLegacyJsonMigration);
});
